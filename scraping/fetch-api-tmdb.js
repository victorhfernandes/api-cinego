...
exports.getFilmeApiTMDB = async (nmf) => {
  try {
    const resNm = await fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=${nmf}&language=pt-BR`
    );
  ...
  try {
    const resId = await fetch(
      `https://api.themoviedb.org/3/movie/${movieID}?api_key=${apiKey}&append_to_response=credits,release_dates&language=pt-BR`
    );
    ...
    let direcao = jsonId.credits.crew.filter(
    ...
    let classificacaoEtaria = jsonId.release_dates.results.filter(
      (element) => element.iso_3166_1 === "BR"
    );
    try {
      dsClassificacaoEtaria =
        classificacaoEtaria[0].release_dates[0].certification;
    } catch {
      dsClassificacaoEtaria = "";
    }
